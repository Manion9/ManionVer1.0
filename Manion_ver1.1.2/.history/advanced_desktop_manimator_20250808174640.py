...
class ManimatorAdvancedApp:
    ...
    def setup_input_panel(self, parent):
        ...
        self.generate_btn = ttk.Button(file_frame, text="🚀 워크플로우 시작", 
                                     command=self.start_workflow, style='Accent.TButton')
    ...
    def create_edit_panel(self, panel_type, title):
        ...
        save_btn = ttk.Button(button_frame, text="💾 저장", 
                             command=lambda: self.save_edit(panel_type))
    ...
    def save_edit(self, panel_type):
        ...
        if panel_type in self.edit_panels:
            ...
            messagebox.showinfo("저장 완료", f"{panel_type} 편집이 저장되었습니다.")
    ...
    def real_handwriting_recognition(self, file_path):
        """실제 손글씨 인식 (실제 구현 시 API 호출)"""
        time.sleep(1)
        filename = os.path.basename(file_path)
        
        # 실제로는 GPT Vision API 호출
        if "equation" in filename.lower() or "math" in filename.lower():
            return f"""수학 방정식 인식 결과 ({filename}):
...
4. 모델 평가 지표: 정확도, 정밀도, 재현율"""
    ...
    def real_storyboard_generation(self, content):
        """실제 스토리보드 생성"""
        time.sleep(2)
        return f"""**Topic**: {content[:30]}...의 시각화

**Key Points**:
* 핵심 개념 1: 기본 정의와 원리 설명
* 핵심 개념 2: 수학적 공식 및 증명 과정
* 핵심 개념 3: 실제 예제를 통한 적용
* 핵심 개념 4: 결론 및 응용 분야
...
**Style**: 3Blue1Brown 스타일, 교육적이고 시각적으로 매력적인 표현"""
