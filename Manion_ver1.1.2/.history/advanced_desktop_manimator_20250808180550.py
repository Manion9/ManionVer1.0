...
"""
        ...
        ttk.Label(key_frame, text="API 키:").pack(anchor="w")
        key_entry = ttk.Entry(key_frame, show="*", width=60)
        key_entry.pack(fill=tk.X, pady=5)
        
        if current_key:
            key_entry.insert(0, current_key[:8] + "..." + current_key[-8:] if len(current_key) > 16 else current_key)
        ...
        def save_key():
            ...
            if key and key.startswith("sk-"):
                ...
                messagebox.showinfo("성공", "API 키가 임시로 설정되었습니다!")
                settings_window.destroy()
            else:
                messagebox.showerror("오류", "올바른 OpenAI API 키를 입력하세요 (sk-로 시작)")
        
        def test_key():
            key = key_entry.get().strip()
            if key:
                os.environ["OPENAI_API_KEY"] = key
                messagebox.showinfo("테스트", "API 키가 설정되었습니다. 이미지 인식을 시도해보세요!")
            else:
                messagebox.showwarning("경고", "API 키를 먼저 입력하세요")
